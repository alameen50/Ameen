...
export default function Footer() {
    return (
        <footer className="bg-white border-t border-navy-50 pt-24 pb-12 overflow-hidden relative transition-colors duration-500">
            <div className="max-w-7xl mx-auto px-6 sm:px-8 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 lg:gap-8 pb-16 border-b border-navy-50">
                    <div className="space-y-8">
                        ...
                        <p className="text-navy-600 font-medium leading-relaxed max-w-xs">
                ...
                <div className="pt-12 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="flex flex-col md:flex-row items-center gap-4 md:gap-8">
                        <p className="text-[11px] font-bold text-navy-400 uppercase tracking-wider">© 2026 Ameen Digital MFB</p>
                        <span className="hidden md:block w-1.5 h-1.5 bg-navy-100 rounded-full" />
                        <p className="text-[11px] font-bold text-emerald-600 uppercase tracking-wider">Regulated by CBN • NDIC Insured</p>
                    </div>
...
function FooterLink({ href, children }) {
    return (
        <a href={href} className="text-[13px] font-bold text-navy-600 hover:text-navy-900 transition-colors">
            {children}
        </a>
    )
}
...
function SocialLink({ icon: IconComponent, href }) {
    return (
        <a
            ...
            className="w-10 h-10 bg-navy-50/50 rounded-xl flex items-center justify-center text-navy-400 hover:bg-emerald-500 hover:text-white transition-all shadow-sm"
        >
